...
def interpolation(procedure, image, n_drawables, drawables, force_cpu, progress_bar, gio_file, config_path_output):
    ...
    with open(os.path.join(weight_path, '..', 'gimp_ml_run.pkl'), 'wb') as file:
        pickle.dump({"force_cpu": bool(force_cpu), "gio_file": str(gio_file), "inference_status": "started"}, file)
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        python_path = config_path_output["python_path"]
        config_path_output["plugin_path"] = os.path.join(config_path, 'interpolation.py')
        ...
        use_header_bar = Gtk.Settings.get_default().get_property("gtk-dialogs-use-header")
        dialog = GimpUi.Dialog(use_header_bar=use_header_bar, title=_("interpolation..."))
        dialog.add_button("_Cancel", Gtk.ResponseType.CANCEL)
        ...
        while True:
            ...
            if response == Gtk.ResponseType.OK:
                ...
                result = interpolation(procedure, image, n_drawables, layer, force_cpu, progress_bar, gio_file, config_path_output)
