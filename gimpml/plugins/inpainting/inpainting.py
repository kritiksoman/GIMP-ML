...
_ = gettext.gettext
image_paths = {"logo": os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'images',
                                    'plugin_logo.png'),
...
model_name_enum = StringEnum(
    "Places2", _("Places2"),
    "CelebA", _("CelebA"),
    "Paris-StreetView", _("Paris-StreetView")
)
...
def inpainting(procedure, image, n_drawables, drawables, force_cpu, model_name, progress_bar, config_path_output):
    ...
    with open(os.path.join(weight_path, '..', 'gimp_ml_run.pkl'), 'wb') as file:
        pickle.dump({"force_cpu": bool(force_cpu), "n_drawables": n_drawables, "model_name": model_dict[model_name],
...
def run(procedure, run_mode, image, n_drawables, layer, args, data):
    ...
    if run_mode == Gimp.RunMode.INTERACTIVE:
        ...
        if n_drawables > 2:
            show_dialog("Please select only image layer and mask layer.", "Error !", "error")
        ...
        while True:
            ...
            if response == Gtk.ResponseType.OK:
                ...
                result = inpainting(procedure, image, n_drawables, layer, force_cpu, model_name, progress_bar,
                                    config_path_output)
