...
#                return eval(self.get_text(), {}, {})
    ...
    def f(dlg, params):
        # output, output_shape = run_script(params)
        gobject.idle_add(run_script, params)
        gobject.idle_add(on_slow_function_done, dlg)
        gobject.idle_add(progress.set_text, "Done")

    def on_slow_function_done(dlg):
        # Re-enable the button and update the label
        dlg.set_response_sensitive(gtk.RESPONSE_OK, True)
        dlg.set_response_sensitive(gtk.RESPONSE_CANCEL, True)
        return False  # Return False to stop calling this function

    def response(dlg, id):
        if id == gtk.RESPONSE_OK:
            progress.set_text("Please wait.")
            dlg.set_response_sensitive(gtk.RESPONSE_OK, False)
            dlg.set_response_sensitive(gtk.RESPONSE_CANCEL, False)
            dlg.set_response_sensitive(gtk.RESPONSE_HELP, False)
            params = []
            ...
            try:
                ...
                try:
                    # th_run = threading.Thread(target=f, args=(dlg, params))
                    # th_run.start()
                    gobject.idle_add(run_script, params)
                    gobject.idle_add(on_slow_function_done, dlg)
                    gobject.idle_add(progress.set_text, "Done")
